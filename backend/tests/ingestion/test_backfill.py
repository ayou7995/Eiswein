...
def _seed_user_with_symbol(session_factory: sessionmaker[Session], symbol: str) -> int:
    with session_factory() as session:
        user = User(username=f"user_{symbol}", password_hash="x", is_admin=False)
        session.add(user)
        session.flush()
        session.add(Watchlist(user_id=user.id, symbol=symbol, data_status="pending"))
        session.commit()
        return user.id
...
async def test_backfill_marks_delisted_when_frame_empty(
    ...
    with session_factory() as session, pytest.raises(DataSourceError):
        await backfill_ticker("DELIST", user_id=user_id, db=session, data_source=ds)
...
async def test_backfill_marks_failed_on_upstream_error(
    ...
    with session_factory() as session, pytest.raises(DataSourceError):
        await backfill_ticker("BADTK", user_id=user_id, db=session, data_source=ds)
...
async def test_backfill_serializes_concurrent_calls_for_same_symbol(
    ...
    async def _run(uid: int) -> str:
        with session_factory() as sess:
            return await backfill_ticker("SPY", user_id=uid, db=sess, data_source=ds)
